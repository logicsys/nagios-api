#!/usr/bin/env bash
# Build a .deb package for nagios-api inside a container.
set -euo pipefail
...
RUN apt-get update && \
    apt-get install -y --no-install-recommends ruby ruby-dev gcc make && \
...
cat > \$STAGING/lib/systemd/system/nagios-api.service <<'UNIT'
[Unit]
...
cat > \$STAGING/etc/default/nagios-api <<'CONF'
...
fpm -s dir -t deb \
    ...
    --config-files /etc/default/nagios-api \
    --deb-systemd-enable \
